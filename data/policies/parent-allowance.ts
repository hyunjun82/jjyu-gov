/* ── 부모급여 (2026) 허브 데이터 — generic 렌더링용 ──
 * 출처: 대한민국 정책브리핑 카드뉴스 (2026.01.14)
 * URL: https://www.korea.kr/multi/visualNewsView.do?newsId=148957936
 * 주관: 보건복지부
 * 검수: 2026-05-13
 */

export const parentAllowancePolicy = {
  id: '3',
  type: 'cash' as const,
  title: '부모급여 지급일과 신청방법, 어린이집 이용하면 어떻게 되나요?',
  titleKeywords: {
      "k1": "부모급여"
  },

  slug: 'parent-allowance',
  org: '보건복지부',
  cat: '지원금',
  catSlug: 'fund',
  amount: '0세 월 100만원 / 1세 월 50만원',
  deadline: '상시 (출생 60일 이내 시 소급)',
  views: 245100,
  applyUrl: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657',
  ctaLabel: '부모급여 자격 확인하고 신청하기 (복지로 공식)',
  datePublished: '2026-01-14T09:00:00+09:00',
  dateModified: '2026-05-23T09:00:00+09:00',
  summary:
    '만 2세 미만 모든 아동에게 0세 월 100만원, 1세 월 50만원을 현금으로 지급하는 영아 양육 지원 제도. 소득기준 없이 전국 모든 영아가 대상이며, 어린이집을 이용하는 경우 영유아 기본 보육료를 제외한 차액을 받습니다.',
  metaDescription: '부모급여 지급일과 신청방법, 어린이집 이용하면 어떻게 되나요? – 0세 월 100만원 / 1세 월 50만원. 만 2세 미만 모든 아동에게 0세 월 100만원, 1세 월 50만원을 현금으로 지급하는 영아 양육 지원 제도.',
  audience: '만 2세 미만 (만 0~1세) 모든 아동',

  keyFacts: {
    지원유형: '현금 지원 (가정양육 매월 25일 지급)',
    지원대상: '만 2세 미만 모든 아동',
    지원금액: '만 0세 월 100만원 · 만 1세 월 50만원',
    소득기준: '없음 (전국 모든 영아)',
    어린이집이용: '영유아 기본 보육료 제외 차액만 현금 지급',
    가정양육지급일: '매월 25일',
    차액지급일: '익월 20일',
    신청방법: '행정복지센터·복지로·정부24',
    문의: '보건복지상담센터 129',
  },
  keyFactsHighlights: {
    지원대상: ['만 2세 미만'],
    지원금액: ['100만원', '50만원'],
    소득기준: ['없음'],
    가정양육지급일: ['매월 25일'],
    차액지급일: ['익월 20일'],
  },

  // Q&A 7개 (cash 유형 표준 + 부모급여 본문 데이터)
  qa: [
    {
      q: '부모급여 지급 금액은 얼마인가요?',
      anchor: 'q-amount',
      act: { cue: '0세와 1세가 두 배 차이입니다. 우리 아이 개월 수로 얼마인지부터 확인하세요.', label: '내 지급액 확인하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '2026년 부모급여는 만 0세(0~11개월)에게 월 100만원, 만 1세(12~23개월)에게 월 50만원을 현금으로 지급합니다. 소득·재산 기준이 전혀 없어 전국 모든 영아가 지원 대상입니다. 어린이집 이용 시에도 영유아 기본 보육료를 제외한 차액이 현금으로 지급되며, 2024년부터 2026년까지 지급 금액은 동일하게 유지되고 있습니다. 본 정책은 보건복지부 공식 자료에 기반하며 최신 정보는 복지로(bokjiro.go.kr)에서 확인하세요.',
      highlights: ['만 0세는 월 100만원', '만 1세는 월 50만원'],
      table: {
        caption: '연령별 부모급여 지급액 (2026년)',
        headers: ['연령', '월 지급액', '변동 사항'],
        rows: [
          ['만 0세 (0~11개월)', '100만원', '전년 동일'],
          ['만 1세 (12~23개월)', '50만원', '전년 동일'],
        ],
      },
      box: {
        label: '소득기준 없음',
        content: '부모급여는 소득·재산과 관계없이 만 2세 미만 모든 아동에게 지급됩니다. 신청만 하면 받을 수 있습니다.',
      },
      sourceNote: '* 출처: 보건복지부 카드뉴스 (2026.01.14)',
    },
    {
      q: '누가 받을 수 있나요?',
      anchor: 'q-eligibility',
      act: { cue: '소득은 안 보는데 국적·주민등록에서 걸려 못 받는 경우가 있습니다. 우리 아이가 해당되는지 1분이면 대조됩니다.', label: '지원대상 요건 대조하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '대한민국 국적의 만 2세 미만(0~23개월) 아동이라면 소득·재산 기준 없이 누구나 받을 수 있습니다. 출생신고가 완료된 아동이 가정에서 양육되는 경우 신청 가능하며, 아동복지시설 보호 아동은 별도 지원 체계가 적용됩니다. F-5 영주권자 자녀도 일부 인정됩니다. 정확한 수급 자격은 복지로(bokjiro.go.kr) 또는 보건복지상담센터 129에 문의하시면 됩니다. 본 정책은 보건복지부 공식 자료에 기반합니다.',
      highlights: ['만 2세 미만', '소득기준 없음'],
      box: {
        label: '필수 요건',
        content: '· 출생신고 완료된 아동\n· 대한민국 국적 또는 인정 영주권자\n· 가정에서 양육 중 (시설 보호 아동 제외)',
      },
      sourceNote: '* 출처: 보건복지부 카드뉴스 (2026.01.14)',
    },
    {
      q: '어떻게 신청하나요?',
      anchor: 'q-apply',
      act: { cue: '출생 후 60일을 넘기면 그 전 달들은 소급되지 않습니다. 출생신고와 같이 접수하면 한 번에 끝납니다.', label: '복지로에서 신청 접수하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '행정복지센터 방문 또는 복지로·정부24 온라인으로 신청할 수 있습니다. 출생신고와 동시에 부모급여를 원스톱으로 신청하면 가장 편리합니다. 소득기준이 없어 별도 소득 서류가 필요 없으며 신분증·출생증명서·통장사본·주민등록등본만 준비하면 됩니다. 신청 다음 달부터 매월 25일 본인 계좌로 자동 입금됩니다. 신청 절차는 변경될 수 있으니 보건복지부 공식 사이트에서 최종 확인이 필요합니다.',
      highlights: ['행정복지센터', '복지로', '정부24', '원스톱'],
      box: {
        label: '방문 신청',
        content: '전국 읍·면·동 행정복지센터 (출생신고와 함께 원스톱 신청)',
      },
      box2: {
        label: '온라인 신청',
        content: '복지로 (bokjiro.go.kr) 또는 정부24 (gov.kr)',
      },
      sourceNote: '* 출처: 보건복지부 카드뉴스 (2026.01.14)',
    },
    {
      q: '지급일은 언제인가요?',
      anchor: 'q-date',
      act: { cue: '25일에 입금이 없다면 어딘가에서 멈춘 겁니다. 처리 단계가 어디인지 조회하면 바로 나옵니다.', label: '신청 처리 상태 조회하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '부모급여는 가정에서 양육하는 경우 매월 25일, 어린이집 이용 아동의 차액은 익월 20일에 지급됩니다. 25일이 주말·공휴일이면 직전 영업일에 입금됩니다. 만 1세 아동이 어린이집을 이용하면 보육료(51만5천원)가 부모급여(50만원)를 초과하여 차액이 0원이 되므로 현금 수령이 없습니다. 신청 후 지급 시작까지 1개월이 소요되며, 지급일은 복지로에서 확인할 수 있습니다.',
      highlights: ['매월 25일', '익월 20일'],
      table: {
        caption: '양육 형태별 지급일',
        headers: ['양육 형태', '지급일'],
        rows: [
          ['가정 양육 아동', '매월 25일'],
          ['어린이집 이용 아동 (차액)', '익월 20일'],
        ],
      },
      sourceNote: '* 25일이 주말·공휴일이면 직전 영업일 지급',
    },
    {
      q: '어린이집 이용 시 어떻게 되나요?',
      anchor: 'q-childcare',
      act: { cue: '어린이집을 보내면 보육료를 따로 신청해야 차액이 나옵니다. 안 하면 바우처도 차액도 안 들어옵니다.', label: '영유아 보육료 신청하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '어린이집을 이용해도 부모급여를 받을 수 있지만, 영유아 기본 보육료가 차감됩니다. 2026년 기준 만 0세는 부모급여 100만원에서 보육료 58만4천원을 뺀 41만6천원을, 만 1세는 부모급여 50만원에서 보육료 51만5천원을 빼면 차액이 없어 현금을 받지 못합니다. 시간제 보육은 별도 기준이 적용됩니다. 보육료는 매년 변경될 수 있으니 보건복지부 공식 사이트에서 최종 확인이 필요합니다.',
      highlights: ['차액만 현금 지급', '41만 6천원', '차액 없음'],
      table: {
        caption: '어린이집 이용 시 차액 계산',
        headers: ['연령', '기본 부모급여', '영유아 기본 보육료', '현금 차액'],
        rows: [
          ['만 0세', '100만원', '58만 4천원', '41만 6천원'],
          ['만 1세', '50만원', '51만 5천원', '없음'],
        ],
      },
      box: {
        label: '주의',
        content: '아동이 어린이집 또는 종일제 아이돌봄을 이용한다면 영유아보육료·종일제 아이돌봄 서비스를 별도로 신청해야 합니다.',
      },
      sourceNote: '* 출처: 보건복지부 카드뉴스 (2026.01.14)',
    },
    {
      q: '늦게 신청하면 소급되나요?',
      anchor: 'q-retro',
      act: { cue: '소급이 안 되는 제도라 늦게 신청한 달은 사라집니다. 아직이라면 오늘 접수하는 게 이득입니다.', label: '지금 신청 접수하기', url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657' },
      intro: '출생일로부터 60일 이내에 신청하면 출생한 달부터 소급해서 부모급여를 받을 수 있습니다. 예를 들어 3월 5일 출생 시 4월 30일(60일 이내) 신청하면 3월분부터 지급됩니다. 60일이 경과하면 신청한 달부터만 지급되어 이전 달분은 받을 수 없습니다. 출생신고와 함께 원스톱 신청을 하면 이런 손실을 막을 수 있습니다. 복지로에서 신청 상태를 확인할 수 있으며, 자세한 사항은 보건복지상담센터 129에 문의하세요.',
      highlights: ['출생 60일 이내', '출생월부터 소급'],
      box: {
        label: '60일 지나면?',
        content: '신청한 달부터 지급되며, 이전 달분은 받을 수 없습니다. 출생신고 시 부모급여까지 한 번에 신청하는 것이 안전합니다.',
      },
      sourceNote: '* 출처: 보건복지부 카드뉴스 (2026.01.14)',
    },
    {
      q: '아이돌봄서비스와 중복 가능한가요?',
      anchor: 'q-overlap',
      act: { cue: '부모급여만 받고 첫만남이용권 200만원·아동수당 10만원을 빠뜨리는 집이 있습니다. 각각 신청해야 각각 나옵니다.', label: '첫만남이용권 신청 상태 점검하기', url: 'https://www.gov.kr/portal/service/serviceInfo/135200005015' },
      intro: '부모급여와 아이돌봄서비스는 별개 제도로 중복 신청이 가능합니다. 단, 종일제 아이돌봄을 이용하는 경우에는 부모급여에서 종일제 이용 비용 일부가 차감될 수 있습니다. 시간제 아이돌봄은 부모급여와 완전 중복 가능합니다. 아이돌봄 외에도 아동수당(만 8세 미만 월 10만원)은 부모급여와 동시 수령이 가능합니다. 정확한 중복 수혜 여부는 129 상담센터에 문의하거나 보건복지부 공식 사이트에서 확인하세요.',
      highlights: ['별도 신청', '종일제 아이돌봄'],
      box: {
        label: '주의 사항',
        content: '종일제 아이돌봄을 이용하는 경우 부모급여 일부가 차감될 수 있습니다. 두 제도 모두 별도 신청이 필요합니다.',
      },
      sourceNote: '* 자세한 사항은 보건복지상담센터 129 문의',
    },
  ],

  details: [
    { label: '지원대상', value: '만 2세 미만 모든 아동 (만 0~1세)' },
    { label: '소득기준', value: '없음 (전국 모든 영아)' },
    { label: '지원유형', value: '현금 지원 (어린이집 이용 시 차액만)' },
    { label: '지원금액', value: '0세: 월 100만원 / 1세: 월 50만원' },
    { label: '신청방법', value: '읍·면·동 행정복지센터 방문 또는 복지로/정부24 온라인' },
    { label: '신청시기', value: '상시 (출생 60일 이내 신청 시 출생월부터 소급지원)' },
    { label: '지급일', value: '가정양육 매월 25일 / 어린이집 차액 익월 20일' },
    { label: '문의처', value: '보건복지상담센터 129' },
  ],

  eligibility: [
    {
      q: '아동이 만 2세 미만 (만 0~1세) 인가요?',
      key: 'age',
      passTip: '연령 조건 충족',
      failTip: '만 0~1세 아동만 부모급여 대상',
    },
    {
      q: '대한민국 국적 또는 영주권 자녀인가요?',
      key: 'nationality',
      passTip: '국적 조건 충족',
      failTip: '대한민국 국적 아동만 대상 (F-5 영주권 일부 인정)',
    },
    {
      q: '출생신고를 완료했나요?',
      key: 'birthreg',
      passTip: '출생신고 완료',
      failTip: '출생신고가 선행되어야 부모급여 신청 가능',
    },
    {
      q: '가정에서 양육 중인가요?',
      key: 'inhome',
      passTip: '가정 양육 확인',
      failTip: '아동복지시설 보호 아동은 별도 지원 체계 적용',
    },
  ],

  incomeTable: [],

  applySteps: [
    { title: '신청 방법 선택', desc: '방문 (읍·면·동 행정복지센터) 또는 온라인 (복지로·정부24)' },
    { title: '출생신고와 함께 원스톱 신청', desc: '출생신고 시 행정복지센터에서 부모급여를 동시 신청' },
    { title: '소득증빙 불필요', desc: '소득기준이 없으므로 소득 관련 서류 제출 불필요' },
    { title: '계좌 등록 + 지급 시작', desc: '신청 다음 달부터 매월 25일 자동 입금' },
  ],

  documents: ['신분증 (부 또는 모)', '출생증명서', '본인 명의 통장사본', '주민등록등본'],

  related: [
    { id: 'child-allowance', title: '아동수당' },
    { id: 'birth-care-employment-grant', title: '첫만남 이용권' },
  ],

  faq: [
    {
      q: '소득기준이 없다는데 정말 모든 아동이 받나요?',
      a: '네. 부모급여는 만 2세 미만 모든 아동에게 지급되며 소득·재산 기준이 없습니다. 신청만 하면 받을 수 있습니다. 시설 보호 아동은 별도 지원 체계로 운영됩니다.',
      source: '보건복지부 카드뉴스 (2026.01.14)',
      sourceUrl: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
    {
      q: '어린이집을 보내면 부모급여를 못 받나요?',
      a: '받을 수 있습니다. 다만 어린이집 이용 시 영유아 기본 보육료를 제외한 차액만 현금 지급됩니다. 0세는 41만 6천원(100만-58.4만), 1세는 차액이 0원입니다.',
      source: '보건복지부 카드뉴스 (2026.01.14)',
      sourceUrl: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
    {
      q: '출생 후 늦게 신청하면 못 받나요?',
      a: '출생일로부터 60일 이내에 신청하면 출생 월부터 소급해서 받을 수 있습니다. 60일이 지나면 신청한 달부터 지급됩니다.',
      source: '보건복지부 카드뉴스 (2026.01.14)',
      sourceUrl: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
    {
      q: '아이돌봄서비스랑 중복으로 받을 수 있나요?',
      a: '두 제도는 별개로 동시 신청 가능하지만, 종일제 아이돌봄을 이용하면 부모급여 일부가 차감될 수 있습니다. 별도 신청 필요.',
      source: '보건복지부 카드뉴스 (2026.01.14)',
      sourceUrl: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
    {
      q: '지급일이 정확히 언제인가요?',
      a: '가정 양육은 매월 25일, 어린이집 이용 차액은 익월 20일에 입금됩니다. 25일이 주말·공휴일이면 직전 영업일에 지급됩니다.',
      source: '보건복지부 카드뉴스 (2026.01.14)',
      sourceUrl: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
  ],

  sources: [
    {
      label: '보건복지부 정책브리핑 카드뉴스 「2026년 부모급여, 이렇게 지원합니다」 (2026.01.14)',
      url: 'https://www.korea.kr/multi/visualNewsView.do?newsId=148957936',
    },
    {
      label: '보건복지부 공식 사이트',
      url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657',
    },
    {
      label: '복지로 (신청 페이지)',
      url: 'https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId=WLF00004657',
    },
  ],
};

export const parentAllowanceSpokes = [
  { slug: '지급금액',    role: 'amount',  title: '2026년 부모급여 0세 100만원·1세 50만원, 아동수당까지 같이 신청되나' },
  { slug: '어린이집전환', role: 'usage',   title: '어린이집 보내면 부모급여 못 받나 | 보육료 차액 계산과 전환 기준' },
  { slug: '신청지급일',  role: 'apply',   title: '부모급여 신청 방법과 지급일 | 출생 후 60일 기한 놓치면 소급 안 된다' },
  { slug: '육아휴직중복', role: 'overlap', title: '육아휴직급여 받으면서 부모급여도 받을 수 있나 | 중복 수급 정리' },
];
