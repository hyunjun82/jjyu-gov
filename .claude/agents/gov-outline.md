...
- **한 틀을 돌려쓰지 마라.** 동사를 확인·조회·계산·청구·발급·비교·챙기기 로 흩는다.
  같은 끝 어절이 겹치면 `check-button-variety.ts` 가 push 를 막는다.
- 문구는 **바로 앞 문장에서 나온다.** 서류 얘기를 안 했으면 "서류 챙기기"는 와닿지 않는다.
- **소리 내어 읽어 어색하면 버린다.** 사람이 안 쓰는 말은 버튼에 쓰지 않는다.
  금지(2026-08-23 지적): `덮는지` `가려내기` `갈라보다`. 새로 지적받으면 여기에 붙인다.
- **목적지를 문구에 맞춘다.** 서류로 끝나면 서류 스포크, 금액이면 계산 스포크.
  목적지가 없는데 "신청하기"라고 쓰면 거짓말이다.

## 목적지 — 여기서 2026-08-23 에 틀렸다

**이미 쓴 글 목록에서 고르지 마라.** registry·허브 배열을 훑어 가장 가까운 것을 집는 건 금지다.
간병인보험 글의 상단 버튼이 실비 세대 비교 허브로 갔다. 링크는 살아 있었고, 독자에겐 엉뚱했다.

순서는 이것뿐이다.

1. 이 소제목을 읽고 난 독자가 **다음에 하려는 행동**을 한 줄로 적는다
2. 그 행동을 받아주는 페이지가 우리에게 있는지 찾는다
3. **없으면 `href: null`** 로 두고, 표에 `목적지 없음 — 물어볼 것` 이라고 적는다

3번을 부끄러워하지 마라. 없는 걸 있는 척 걸면 그게 사고다.
