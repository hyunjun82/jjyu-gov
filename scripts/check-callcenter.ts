...
const INDUSTRY: Record<string, { dir: string; label: string; own: string[] }> = {
  ...
  online: {
    dir: '온라인고객센터',
    label: '온라인 서비스',
    /* '긴급출동' 은 보험 용어이면서 자동차 제조사 원문 라벨이다 (2026-09-26 현대자동차 '긴급출동, 안전 보안 서비스는 365일 24시간 제공').
       자동차 틀이 없어 온라인 틀로 쓰므로 여기서는 금지어에서 뺀다. 증권·카드 글에서는 여전히 잡힌다. */
    own: ['주문번호', '환불', '배송', '구독', '긴급출동'],
  },
  /* 대출 고객센터 (2026-08-27 신설).
     같은 회사라도 대표번호와 대출 창구가 갈린다 —
     국민은행 대표 1588-9999 / 대출 1588-9008, 웰컴저축은행 수신 1661-9400 / 여신 1661-0001.
     그래서 "삼성화재 고객센터" 와 "삼성화재 대출 고객센터" 는 다른 글이다.
     여기 넣은 말은 다른 업종에서 금지어가 된다 — 보험 글에 "중도상환수수료" 가 나오면 잡힌다. */
  loan: {
    dir: '대출고객센터',
    label: '대출 창구',
    /* "여신"·"상환" 은 뺐다 — 대출 전용어가 아니라 원문 라벨로 다른 업종 글에도 정당하게 나온다.
       교보생명 공식 안내에 "여신(대출) 1588-1010" 이 그대로 적혀 있다.
       원문에 있는 말을 금지어로 잡으면 게이트가 옳은 글을 막는다. */
    /* 카드론·현금서비스는 카드사 용어이면서 대출이기도 하다.
       롯데카드 원문 라벨이 "금융서비스(현금서비스·대출상담)" 이고
       광주은행 카드가 "카드론·교차상품판매" 다 — 대출 글에 나오는 게 맞다.
       여기 적어 두면 이 업종에서는 금지어에서 빠진다(mineWords).
       보험·증권 글에 나오면 여전히 잡힌다. */
    own: ['중도상환수수료', '만기연장', '금리인하요구권', '카드론', '현금서비스'],
  },
  /* 공공기관 (2026-09-01 신설).
     회사가 아니라 기관이다 — 걸리는 지점이 "번호가 갈린다" 가 아니라
     "업무시간이 지나면 창구가 닫히고 남는 게 기관마다 다르다" 다.
     여기 넣은 말은 다른 업종에서 금지어가 된다 — 보험 글에 "탈세제보" 가 나오면 잡힌다. */
  public: {
    dir: '공공기관고객센터',
    label: '공공기관',
    /* "증명서 발급"·"셀프서비스" 는 뺐다 — 보험사 원문에도 그대로 나온다
       (AXA·DB·하나생명·흥국화재·현대해상 5곳을 이 목록이 잡았다).
       원문에 있는 말을 금지어로 잡으면 게이트가 옳은 글을 막는다. */
    /* '사고접수' 는 보험 용어이면서 도시가스 원문 라벨이다 (2026-09-26 참빛충북도시가스
       '휴일 / 야간 24시간 긴급사고접수 (상황통제실 1899-9100 전화 후 “3” 선택)'). 도시가스는 공공 틀이라
       여기 적어 공공 글에서는 금지어에서 뺀다. 증권·카드·온라인 글에서는 여전히 잡힌다. */
    own: ['탈세제보', '산재보상', '민원서류', '사고접수'],
  },
  /* 가전제품 AS (2026-09-08 신설). 회사가 아니라 브랜드고, 급할 때가 '고장'이다.
     여기 넣은 말은 다른 업종에서 금지어가 된다. */
  appliance: {
    dir: '가전고객센터',
    label: '가전업체',
    /* '배송'·'구독' 은 온라인 용어이면서 가전 원문 라벨이기도 하다 (2026-09-26 쿠쿠
       "렌탈(구독) A/S, 설치, 점검, 요금 문의" · "쿠쿠몰 제품 구매 및 배송 문의").
       원문에 있는 말이라 여기 적어 가전 글에서는 금지어에서 뺀다. 보험·증권 글에서는 여전히 잡힌다. */
    own: ['출장비', 'AS 접수', '부품 주문', '설치 예약', '배송', '구독'],
  },
};
...
for (const c of all) {
  ...
  if (!fs.existsSync(src)) p.push('추출본이 없다 — 공식 페이지를 뜬 적이 없다');
    ...
    /* 우리가 쓰는 말은 원문에 있을 리 없으니 면제한다.
   "확인하지 못함" 은 대출 창구 상담시간을 원문에서 못 찾았을 때 쓴다 (2026-08-27). */
    const OURS = /표기 없음|미공개|공식 채널 확인|확인 필요|확인하지 못함|확인되지 않/;
    Object.entries(c.hours ?? {}).forEach(([k, v]) => {
