...
- ★ **연도가 박힌 보도자료를 1차 출처로 삼지 않습니다** (2026-09-20 추가 — 중간예납 글이 작년 11월 보도자료를 골라 본문 날짜가 통째로 한 해 밀렸습니다). 보도자료·briefing·pressRelease 는 그해 발송일·납부기한·대상 인원이 박혀 있어, 올해(${new Date().getFullYear()}년) 글에 옮기면 전부 틀린 수치가 됩니다. 해마다 바뀌는 날짜·금액·기준을 다루는 주제일수록 **법령 조문(law.go.kr)과 기관의 상시 안내 페이지**를 먼저 고릅니다 — 이런 문서는 "11월 30일까지" 처럼 해가 바뀌어도 맞는 표현을 쓰고, 기관 안내는 올해 기준으로 갱신됩니다. 보도자료는 제도 배경을 덧붙일 때만 보조로 씁니다.
...
- ★ 연도를 먼저 봅니다 (2026-09-20 추가 — 중간예납 1번 글이 작년 보도자료를 그대로 옮겨 본문 전체가 한 해 밀렸습니다). 추출본의 출처마다 배포·작성 연도를 확인해 오늘(${new Date().toISOString().slice(0, 10)})과 견줍니다. 지난해 이전 문서의 **연도에 매인 수치** — 그해 발송일·납부기한·대상 인원·귀속 연도(예: "11.3.(월)부터", "2025년 12월 1일까지", "152만 명", "'24년 귀속") — 는 올해 것처럼 옮기지 않습니다. 작년 자료에서는 해마다 변하지 않는 **제도 설명**만 가져오고, 날짜·기한은 법령의 항구 규정(예: 소득세법이 정한 "11월 30일까지")이나 올해 자료로 씁니다. 올해 근거가 없으면 그 수치는 본문에서 빼고 "notUsing" 에 "작년(YYYY) 자료뿐이라 쓰지 않음" 으로 적습니다.
- 소제목이 묻는 기준(배기량·폐지 여부 등)이 추출본에 없을 때: 소제목은 사장님 것이라 그대로 둡니다. 그 카드는 "현행 원문에는 그 기준이 없고 대신 … 기준만 있다" 를 조문 번호·문장으로 답합니다 — 없다는 사실도 원문 근거가 있는 답입니다. 이것 때문에 작성을 중단하지 않습니다. 중단은 추출본 전체가 주제와 무관할 때만입니다.
...
- 카드마다 intro(500~900자, 검색한 사람 기준의 답. 결론부터, 근거 문장 인용하며) · highlights 3~5개(40자 이내) · 눈으로 보는 것 하나 이상(table 또는 box 또는 list — 글자만 있는 카드는 거부됩니다) · sourceNote("* 출처: 기관 문서명 (${new Date().toISOString().slice(0, 10)} 확인)"). 수치를 담은 카드에는 sourceNote 를 반드시.
- 눈으로 보는 것 고르는 법: 추출본에 기준·금액·구간·가구 유형이 둘 이상 나열돼 있으면 table(2~4열·행 2~5, 글 한 편에 2~3개가 정본의 밀도입니다). 한 덩어리 규정·조문이면 box(원문 문장 그대로 인용, label 에 문서명·조문 번호). 순서·서류·요건 나열이면 list(3~6줄, 한 줄 40자 이내). 표의 칸은 추출본 문자열 그대로 옮깁니다 — 열을 만들어 채우지 않습니다.
...
- 필드 이름은 화면이 읽는 그대로입니다: 상단 버튼은 heroAct: {label, href} — href 입니다(url 아님). 카드 버튼은 act: {cue, label, url}. 핵심콕콕은 keyFacts. 이름을 바꾸면 화면에 아무것도 안 그려지고 화면 검사가 "h1 이 화면에 없다" 로 떨어집니다.
...
- 화면(dev 렌더) 실패 — "h1 이 화면에 없다"·"상단 버튼이 화면에 없다"·"핵심콕콕 박스가 화면에 없다" 가 함께 나오면 글이 아니라 필드 이름이 틀린 것입니다. 문장을 다시 쓰지 말고 heroAct.href·keyFacts·h1 의 이름만 맞춥니다.
- 나머지 부분은 바꾸지 않습니다.
