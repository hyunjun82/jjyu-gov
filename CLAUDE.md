...
**모든 글은 "행동 키워드" 기반으로 작성 — 특히 허브는 무조건** (2026-07-17 확정) — 타이틀·본문이 "정보 전달"에서 끝나면 안 되고, 반드시 사용자가 실제로 누를 수 있는 행동(신청하기·조회하기·다운로드·접수하기 등)으로 귀결돼야 한다.
   ...
   - ⚠️ **"~하는 법"으로 끝내는 건 금지** (2026-07-30 확정). 과거 이 규칙을 "제목 끝에 하는 법을 붙인다"로 잘못 굳혀서 '~법' 종결이 사이트를 도배했다(현황 수치는 `.claude/rules/title-workflow.md` §4가 원천 — 여기 복사하지 않는다). 같은 어미가 사이트 전체에 반복되면 포털이 기계 생성으로 볼 여지가 생긴다. **행동 동사는 문장 안에 두고 종결은 매번 다르게** — 정본 `docs/title-style-24.md` 24개처럼 물음형·시나리오형으로 끝낸다: "티켓 어디서 예매하나", "늦으면 못 받는 이유는?", "며칠 받나", "놓치면 어떻게 되나", "1분이면 끝". `check-click-value.ts`의 D축(종결어미 도배)이 신규 글에서 이를 차단한다.
...
> 🎯 **타이틀 규칙의 단일 정본은 `.claude/rules/title-workflow.md`** (2026-08-08 확정, `app/policy/**`·`data/policies/**` 작업 시 자동 로드 + Write/Edit 훅이 `docs/title-log.md` 기록 없으면 저장 차단). 순서: **캡처(`reference/titles/`) → 검색어 → 소제목 → 타이틀 → 버튼**. 아래 요약이 그 파일과 어긋나면 그 파일이 이긴다.
...
1. **소제목 개수는 타이틀이 정한다** (2026-08-08 확정). 고정 7개가 아니다.
   ...
   빈 글 방지용 하한만 `check-spoke-quality.sh` 가 검사한다 (수치는 그 스크립트가 원천 — 여기 적지 않는다).
...
1. qa[] 질문을 먼저 확정한다 (사용자가 실제로 묻는 말로 — **개수는 타이틀이 정한다, 고정 7개 아님**. 게이트 1번)
...
- **타이틀 워크플로우 정본**(캡처→검색어→소제목→타이틀→버튼, 구성표 양식, 버튼 슬롯 [2,4,마지막]): `.claude/rules/title-workflow.md` — `app/policy/**`, `data/policies/**` 작업 시 자동 로드.
