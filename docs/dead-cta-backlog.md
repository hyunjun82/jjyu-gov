...
## 작업 중 주의 (2026-08-01 겪음)
- **동시 빌드 금지**: `next build`를 두 개 이상 겹쳐 돌리면 같은 `.next`를 써서
  `ENOENT ... pages-manifest.json`으로 실패한다. 코드 문제가 아니므로 한 번에 하나만 돌린다.
- **게이트는 `origin/main...HEAD` diff 범위만 검사**한다. 파일을 고친 뒤 커밋하지 않고 게이트를
  돌리면 "문제 0개"로 통과한 것처럼 보인다. **커밋 후 다시 돌려야** 실제 검사가 된다.
- 자주 걸리는 항목: 목적지 뭉침(한 글의 act.url이 전부 동일), 문구 도배(다른 글과 같은 문장),
  어미 반복(같은 종결 3회), 라벨 정보형("…보기/이수하기"), 타이틀↔소제목 키워드 불일치.
