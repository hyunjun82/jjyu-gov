...
import { MOVEMENTS, REGIONS, byMovement } from '@/lib/patriots';
...
export async function generateMetadata(props: { params: Promise<{ m: string }> }): Promise<Metadata> {
  const { m } = await props.params;
  const mv = decodeURIComponent(m);
  const n = byMovement(mv).length;
  return {
    title: `${mv} 독립유공자 ${n.toLocaleString()}명 명단 — 운동계열별 역사 인물 기록 | 정부지원사업`,
    description: `국가보훈부 공공데이터 기준 ${mv} 계열로 서훈된 독립유공자는 ${n.toLocaleString()}명입니다. 지역별 명단과 훈격을 확인하고 후손 등록 절차로 이어집니다.`,
    alternates: { canonical: `https://gov.jjyu.co.kr/guide/patriots/movement/${encodeURIComponent(mv)}/` },
  };
}

export default async function MovementPage(props: { params: Promise<{ m: string }> }) {
  const { m } = await props.params;
  const mv = decodeURIComponent(m);
  const list = byMovement(mv);
  ...
  return (
    <main style={{ maxWidth: 880, margin: '0 auto', padding: '28px 16px 60px' }}>
      <p style={{ fontSize: 13.5 }}>
        <Link href="/guide/patriots/" style={{ color: '#003D88' }}>독립유공자 명단</Link> › 운동계열별
      </p>
      <h1 style={{ fontSize: 27, fontWeight: 800, color: '#0B2148', margin: '4px 0 6px' }}>
        {mv} 독립유공자 {list.length.toLocaleString()}명
      </h1>
      <p style={{ fontSize: 15, color: '#40507a', lineHeight: 1.7 }}>
        {mv} 계열로 서훈된 독립유공자 명단입니다. 지역(본적)별로 묶어 인원이 많은 곳부터 보여드립니다.
      </p>
      ...
      <section style={{ marginTop: 30 }}>
        <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0B2148' }}>다른 운동계열</h2>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
          {MOVEMENTS.filter(([x]) => x !== mv).slice(0, 8).map(([x, c]) => (
            <Link key={x} href={`/guide/patriots/movement/${encodeURIComponent(x)}/`}
